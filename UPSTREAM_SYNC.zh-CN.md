...
中文默认的 `main` 维护主线以 `vxcontrol/pentagi:main` 为上游基准，保留英文词典作为回退、可切换界面和差异参照。`fulaoaz/pentagi` 汉化版已停止独立更新，本仓库是唯一维护主线：每轮按“合入上游 → 汉化跟进 → 增强层验证 → 发布”完成。
...
- 配置、搜索工具、嵌入模型回归测试；
- 浏览器目标 URL 校验、私有/特殊 IP 分类、Scraper 响应体上限、搜索器响应体上限和 UTF-8 截断回归测试；
- MCP 认证、来源、会话、请求体、Schema、读写权限、拒绝审计与输出上限回归测试；
- `git diff --check`。
...
项目仓库与博客仓库分开维护。每次优化版主线完成一轮可见能力或安全边界更新后，同步更新独立博客仓库（通常为相邻的 `../blog`）中的对应文章，记录实际行为、配置默认值、验证结果和参考项目；不要把独立汉化仓库作为博客或代码更新入口。博客文章更新后，在博客仓库运行：
...
发布顺序保持一致：先在 `fulaoaz/pentagi-optimizer` 完成上游同步、汉化跟进和增强层测试，再更新博客内容与构建产物。这样博客描述始终对应优化版，而不是已经停止独立维护的汉化版。
