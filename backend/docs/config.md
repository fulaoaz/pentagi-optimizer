...
| MCPEnabled       | `MCP_ENABLED`               | `true`                                                                       | Enables the native MCP bridge                                             |
| MCPAPIKey        | `MCP_API_KEY`               | *(none)*                                                                     | Required Bearer credential for MCP endpoints unless anonymous access is explicitly enabled |
| MCPWriteAPIKey   | `MCP_WRITE_API_KEY`         | *(none)*                                                                     | Optional separate Bearer credential for write-capable MCP tools; empty preserves single-key compatibility |
| MCPAllowAnonymous | `MCP_ALLOW_ANONYMOUS`      | `false`                                                                      | Allows unauthenticated access only for an isolated local bridge; it never enables write tools |
| MCPAllowedOrigins | `MCP_ALLOWED_ORIGINS`      | *(none)*                                                                     | Comma-separated browser origins allowed to access MCP; empty denies browser-origin requests while preserving desktop clients |
| MCPAllowedTools | `MCP_ALLOWED_TOOLS`         | *(none)*                                                                     | Optional comma-separated exact tool names; empty exposes all otherwise-enabled tools, a nonempty value enforces least privilege |
| MCPMaxRequestBytes | `MCP_MAX_REQUEST_BYTES`   | `1048576`                                                                    | Maximum MCP HTTP request body size in bytes; nonpositive values use the 1 MiB default |
| MCPReadToolRateLimit | `MCP_READ_TOOL_RATE_LIMIT` | `60`                                                                      | Per-principal, per-tool read calls per minute; `0` disables read-tool limiting |
| MCPWriteToolRateLimit | `MCP_WRITE_TOOL_RATE_LIMIT` | `10`                                                                    | Per-principal, per-tool write calls per minute; `0` disables write-tool limiting |
| MCPApprovalMode | `MCP_APPROVAL_MODE`         | `scope`                                                                      | Approval gate: `scope` (compatibility), `destructive`, or `write`; gated calls require `X-MCP-Approval: confirm` |
| MCPEnableWriteTools | `MCP_ENABLE_WRITE_TOOLS` | `false`                                                                      | Registers `submit_flow_input` and `stop_flow` for authenticated MCP clients |

The installer’s Server Settings screen exposes all MCP variables above. It masks both API keys, validates boolean and request-size values, and normalizes browser-origin and exact-tool allowlists before writing the environment state.

MCP tool schemas are strict by default and validate incoming arguments before handlers run. Flow IDs are positive integers, submitted text is bounded, and tool annotations expose read-only/destructive hints for client-side approval flows. When `MCP_WRITE_API_KEY` is set, read and write scopes are separated; otherwise the existing single-key behavior is preserved. `MCP_ALLOWED_TOOLS` can further reduce the registered surface to an exact comma-separated allowlist; write-tool opt-in remains an independent gate. `MCP_READ_TOOL_RATE_LIMIT` and `MCP_WRITE_TOOL_RATE_LIMIT` apply bounded per-principal, per-tool token buckets; `MCP_APPROVAL_MODE` can require `X-MCP-Approval: confirm` for destructive or all write tools. CORS preflight is allowed only for configured origins; actual messages still require Bearer authentication. Dynamic data from `get_flow_status` is bounded to 64 KiB and labelled as untrusted so clients do not treat embedded flow or task text as instructions. Every MCP tool call produces a structured audit entry containing `tool`, `auth_mode`, `scope`, `access`, `outcome`, `decision`, `duration_ms`, `request_id`, `principal_hash`, `session_hash`, `origin_hash`, and `client_ip_hash`; `decision` differentiates allowed calls from rate-limit, approval, and write-scope denials. The hash fields use the first eight bytes of SHA-256 to correlate requests without recording raw browser origins or client addresses. Tool arguments, result bodies, credentials, raw origins, and raw client addresses are intentionally excluded from these entries. HTTP access logs redact MCP session IDs, tokens, keys, and one-time OAuth query values.
MCP tool schemas are strict by default and validate incoming arguments before handlers run. Flow IDs are positive integers, submitted text is bounded, and tool annotations expose read-only/destructive hints for client-side approval flows. When `MCP_WRITE_API_KEY` is set, read and write scopes are separated; otherwise the existing single-key behavior is preserved. `MCP_ALLOWED_TOOLS` can further reduce the registered surface to an exact comma-separated allowlist; write-tool opt-in remains an independent gate. `MCP_READ_TOOL_RATE_LIMIT` and `MCP_WRITE_TOOL_RATE_LIMIT` apply bounded per-principal, per-tool token buckets; `MCP_APPROVAL_MODE` can require `X-MCP-Approval: confirm` for destructive or all write tools. CORS preflight is allowed only for configured origins; actual messages still require Bearer authentication. Dynamic data from `get_flow_status` is bounded to 64 KiB and labelled as untrusted so clients do not treat embedded flow or task text as instructions. Every MCP tool call produces a structured audit entry containing `tool`, `auth_mode`, `scope`, `access`, `outcome`, `decision`, `duration_ms`, `request_id`, `principal_hash`, `session_hash`, `origin_hash`, and `client_ip_hash`; `decision` differentiates allowed calls from rate-limit, approval, and write-scope denials. The hash fields use the first eight bytes of SHA-256 to correlate requests without recording raw browser origins or client addresses. Tool arguments, result bodies, credentials, raw origins, and raw client addresses are intentionally excluded from these entries. HTTP access logs redact MCP session IDs, tokens, keys, and one-time OAuth query values. When an MCP `submit_flow_input` call creates or resumes asynchronous work, a separate `action=mcp_task_trigger` record includes `flow_id`, `task_id`, `trigger`, and the sanitized correlation fields. The queue copies only this compact object, never the HTTP request context; worker logs retain only `input_bytes`, not input content or a fingerprint. Correlated MCP `terminal` and `file` actions emit `action=mcp_sandbox_action` with flow/task/subtask IDs, tool, operation, outcome, duration, and the same sanitized fields. It excludes commands, file paths, arguments, and output bodies; terminal failure logs retain only `result_bytes`.

Correlated flow-executor provisioning and removal of the primary container emit `action=mcp_container_lifecycle` with `flow_id`, `container_type`, `operation`, `outcome`, `duration_ms`, and the sanitized correlation fields. These entries deliberately exclude container IDs, names, images, work directories, host paths, and Docker API details.

When a persisted primary-container status disagrees with the Docker runtime, the flow executor emits `action=container_recovery` with `flow_id`, `container_type`, `recovery_reason`, `outcome`, and `duration_ms`. The allowed recovery reasons are `runtime_unavailable`, `runtime_stopped`, and `stale_status`; MCP-correlated contexts add the sanitized correlation fields. Removal and reprovision failures are recorded with `outcome=error`. Recovery entries exclude container IDs, names, images, work directories, host paths, Docker error bodies, and other runtime details.
...
### Browser target and response limits

Browser target URLs are validated before they are sent to a scraper. Only `http` and `https` are accepted; a target must have a host, a valid port, and no URL user information. Target URLs longer than `8 KiB` are rejected. Local, private, link-local, unspecified, shared, reserved/documentation, multicast, and other non-public IP ranges are classified as private. For hostnames, every address returned by DNS is checked; if any address is non-public, the private scraper is selected. Known local suffixes and bare service names are treated as private before DNS lookup.

The scraper response body is capped at `16 MiB` and response headers at `64 KiB` before markdown, HTML, links, or screenshot data is parsed or stored. The body limit is enforced for both fixed-length and chunked responses. Search-provider response bodies are capped at `4 MiB` in the shared searcher layer before JSON or HTML parsing, and the EPSS/CVE/KEV intelligence feeds use an `8 MiB` JSON cap. Searcher prompts are bounded to `256 KiB` before they reach a summarizer. These are fixed safety bounds rather than environment settings. Downstream tool output and summarization paths apply their own smaller limits, and external content is explicitly marked as untrusted data.
...
The internal engine inherits the browser's URL validation and `16 MiB` scraper response cap. Its per-site `WEB_SEARCH_INTERNAL_MAX_SITE_BYTES` bound is applied afterward with UTF-8-safe truncation, so a single page cannot consume the full summarization budget.
